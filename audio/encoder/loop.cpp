...
#define minlimit				-100.0
	...
	*/
	...
	the_lo_quant = -_infinity_;   /* "-_infinity_" */
	the_hi_quant = -_infinity_;   /* the real maximum for high_quant is about +4 ! */
	...
	for (sfb = 0;  sfb < end_sfb_l;  sfb++)
	{
		...
		if (xmax == 0.0)
		{
			lo_quant_l[sfb] = -_infinity_;
			hi_quant_l[sfb] = -_infinity_;
		}
		else
		{
	...
	for (sfb = min_used_sfb_s;  sfb < end_sfb_s;  sfb++)
	{
		for (b = 0;  b < 3;  b++)
		{
			...
			if (xmax == 0.0)
			{
				lo_quant_s[sfb][b] = -_infinity_;
				hi_quant_s[sfb][b] = -_infinity_;
			}
			else
			{
...
/*
	...
	If there is no suitable index, it returns "_infinity_".
*/
...
)
{
	...
	cod_info->part2_length = _infinity_;
...
static	int 		cutting_crew (FLOAT in)
{
	int						retVal;
